                ...
                <tbody class="list" id="staff02">
                    <template v-if="resellers.length > 0">
                      <tr v-for="(reseller, index) in resellers" :key="reseller.id">
                        ...
                        <td>{{ reseller.brand }}</td>
                        <td>
                          <img v-if="reseller.logo_path" :src="reseller.logo_path" alt="Reseller Logo" class="logo-image" style="width: 50px;">
                          <span v-else>No Logo</span>
                        </td>
                        <td>
                          <router-link :to="{ name: 'admin.reseller.edit', params: { id: reseller.id }}">
                            <button class="btn btn-success mr-1"> <i class="fa fa-pencil-alt"></i></button>
                          </router-link>
                          <router-link :to="{ name: 'admin.reseller.detail', params: { id: reseller.id }}">
                            <button class="btn btn-info mr-1"> <i class="fa fa-eye"></i> </button>
                          </router-link>
                          <button class="btn btn-danger" @click="deleteReseller(reseller.id)"><i class="fa fa-times"></i></button>
                        </td>
                      </tr>
                    </template>
                    <template v-else>
